...
if __name__ == "__main__":
    ...
    sched = torch.optim.lr_scheduler.LambdaLR(
        ...
        lr_lambda=lambda s: (
            ...
            if s < 500
                ...
                0.05, 0.5 * (1 + math.cos(math.pi * (s - 500) / max(1, total - 500)))
            )
        ),
    )
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in tqdm(loader):
            ...
            z_hat, z_tgt = model(
                ctx_text=b["ctx_text"],
                ctx_audio=b["ctx_audio"],
                tgt_text=b["tgt_text"],
                tgt_audio=b["tgt_audio"],
                src_lang=b["src_lang"],
                src_mod=b["src_mod"],
                tgt_lang=b["tgt_lang"],
                tgt_mod=b["tgt_mod"],
                ctx_pad_mask=b["ctx_pad_mask"],
                tgt_pad_mask=b["tgt_pad_mask"],
            )
        ...
        print(
            f"epoch {epoch + 1}/{EPOCHS}  avg {running / len(loader):.4f}  {time.time() - t0:.0f}s"
        )
